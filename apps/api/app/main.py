...
from .schemas import TenantCreate, CustomerCreate, LeadCreate, TenantOut, RegisterRequest, LoginRequest, AuthResponse, UserOut
from .services import create_tenant, create_owner, upsert_customer, create_lead, verify_password, create_access_token
from .config import settings
import jwt

app = FastAPI(title="AI Growth OS API", version="0.3.0")
...
def user_out(user: User) -> UserOut:
    return UserOut(id=user.id, email=user.email, name=user.name, tenant_id=user.tenant_id, role=user.role)
...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    ...
    tenant = create_tenant(db, payload.business_name, payload.slug, payload.industry)
    user = create_owner(db, payload.name, payload.email, payload.password, tenant)
