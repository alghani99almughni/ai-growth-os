...
class MainActivity:ComponentActivity(){
 ...
 private fun acceptCall(api:ApiClient,s:AuthSession,call:PendingCall){status.text="Accepting…";scope.launch{try{api.handoffDecision(s,call.id,true);val m=WebRtcSignalingManager(this@MainActivity,apiInput.text.toString().trim().trimEnd('/'),s.token,call.id,s.tenantId,api.iceServers(s.tenantSlug),onConnected={runOnUiThread{status.text="Connected to "+call.customerName}},onEnded={runOnUiThread{status.text="Call ended"};scope.launch{runCatching{api.releaseCall(s,call.id)}}});manager?.close();manager=m;m.connectAndOffer();status.text="Connecting to "+call.customerName+"…"}catch(t:Throwable){status.text=t.message?:"Unable to answer"}}}
